...
node.set_unless['wordpress']['db']['password'] = 'passw0rd'
node.set_unless['wordpress']['keys']['auth'] = 'cvgyLFCiQ9tHVcLyxRWn'
node.set_unless['wordpress']['keys']['secure_auth'] = 'Sw5THXQoFlgcUbKNkcsL'
node.set_unless['wordpress']['keys']['logged_in'] = 'WLNR0paEhRtFMWV0OluI'
node.set_unless['wordpress']['keys']['nonce'] = 'XRejM6ODWblU446V0VWg'
...
cookbook_file "#{node['wordpress']['dir']}/wp-content/plugins/captcha.3.0.tar.gz" do
  source "captcha.3.0.tar.gz"
  owner "root"
  group "root"
  mode "0755"
end  

execute "untar-captcha" do
    cwd node['wordpress']['plugindir']
    command "tar -xzf captcha.3.0.tar.gz"
end
...
template "#{node['wordpress']['dir']}/wp-config.php" do
  ...
  variables(
    ...
    :homeurl         => node['wordpress']['homeurl'],
    :siteurl         => node['wordpress']['siteurl']
  )
